...
The connection handles the selected tab's title and URL, requested source, validation results, execution status, and emulator screen captures. Supported operations include reading and editing source, validating programs, and running and stopping programs. Access starts only after you open the extension popup, review the data-access disclosure, enter the local bridge port and pairing code, and choose "Connect this tab." The port and pairing code are stored locally so the requested connection can be restored.
...
選択したタブのタイトルとURL、要求されたソース、検証結果、実行状態、エミュレーター画面を取り扱い、ソースの読み書き、検証、実行・停止に対応します。拡張機能のポップアップでデータ取扱いの説明を確認し、ローカル接続情報を入力して「Connect this tab」を押したタブだけにアクセスします。ポートとペアリングコードは、要求された接続を復元するためローカルへ保存します。
...
- Website content: X1Pen source, validation results, execution state, and requested emulator screenshots.
...
6. Configure an MCP client using the instructions at `https://github.com/ho-ogino/xmil-web/blob/main/docs/X1PEN_MCP.md` to exercise source read/edit, validation, run/stop, status, and screenshot tools.
