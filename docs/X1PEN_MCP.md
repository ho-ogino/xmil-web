...
- SLANGのsource埋め込み: `$00`などの有効なplaceholder値を選択し、既存の **Import** ボタンでbinary array値へ置換します。byte列をAIへ送る必要はありません。runtime fileとして使う場合はDisk Editorでproject diskへbinaryを追加し、用途に応じて`MAGLOAD`または`FOPEN`／`FREAD`を使います。
