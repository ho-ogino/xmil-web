...
test('bridge requires explicit selection when multiple tabs are connected', async () => {
  ...
  await bridge.selectSession('tab-b');
  const commandMessage = waitForMessage(socket, (message) => message.type === 'command');
...
test('session selection releases pad input on the previous live tab', async () => {
  const bridge = new X1PenBridge({ startPort: 0, pairingCode: '123456', logger: () => {} });
  openBridges.add(bridge);
  await bridge.start();
  const socket = await connectExtension(bridge, {
  ...
  socket.send(JSON.stringify({
    type: 'sessions',
    sessions: [
      { sessionId: 'tab-a', title: 'A', x1pen },
      { sessionId: 'tab-b', title: 'B', x1pen },
    ],
  }));
  await new Promise((resolve) => setTimeout(resolve, 10));
  await bridge.selectSession('tab-a');

  const releaseMessage = waitForMessage(
    socket,
    (message) => message.type === 'command' && message.method === 'releasePads',
  );
  const selection = bridge.selectSession('tab-b');
  const release = await releaseMessage;
  assert.equal(release.sessionId, 'tab-a');
  socket.send(JSON.stringify({
    type: 'result', id: release.id, ok: true, result: { ok: true, released: true },
  }));
  const selected = await selection;
  assert.equal(selected.sessionId, 'tab-b');
  assert.equal(bridge.connectionInfo().selectedSessionId, 'tab-b');
});

test('session selection bounds pad cleanup and requires explicit force to fail open', async () => {
  const bridge = new X1PenBridge({
    startPort: 0, pairingCode: '123456', logger: () => {}, padReleaseTimeoutMs: 20,
  });
  openBridges.add(bridge);
  await bridge.start();
  const socket = await connectExtension(bridge, {
  ...
  await assert.rejects(
    ...
    (error) => error.code === 'PAD_RELEASE_FAILED' && error.previousSessionId === 'tab-a',
  );
  assert.equal(bridge.connectionInfo().selectedSessionId, 'tab-a');

  const forced = await bridge.selectSession('tab-b', { force: true });
  assert.equal(forced.sessionId, 'tab-b');
  assert.equal(forced.warning.code, 'PAD_RELEASE_FAILED');
  assert.match(forced.warning.action, /disconnect\/reload/);
});

test('selected-session disappearance auto-selects the sole survivor without cleanup RPC', async () => {
  const bridge = new X1PenBridge({ startPort: 0, pairingCode: '123456', logger: () => {} });
  openBridges.add(bridge);
  await bridge.start();
  const socket = await connectExtension(bridge);
  ...
  socket.on('message', (data) => {
    const message = JSON.parse(data.toString());
    if (message.type === 'command' && message.method === 'releasePads') cleanupCommands++;
  });
  socket.send(JSON.stringify({
    type: 'sessions',
    sessions: [{ sessionId: 'tab-b', title: 'B' }],
  }));
  await new Promise((resolve) => setTimeout(resolve, 30));
  assert.equal(bridge.connectionInfo().selectedSessionId, 'tab-b');
  assert.equal(cleanupCommands, 0);
});
