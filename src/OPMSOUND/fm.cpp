...
typedef struct ym2151_f {
	UINT8 REGS[256];			/* register mirror (for state save) */
	FM_ST ST;					/* general state     */
...
#endif
		/* clear output acc. */
		out_ch[OUTD_LEFT] = out_ch[OUTD_RIGHT]= 0;
		out_ch[OUTD_CENTER] = (INT32)(*bufL)<<FM_OUTSB; //pickup PSG sound
		/* calcrate channel output */
...
void YM2151SetClock(int num, int clock, int rate)
{
	YM2151 *F2151 = &(FMOPM[num]);

	F2151->ST.clock = clock;
	F2151->ST.rate = rate;
	F2151->ST.freqbase  = rate ? ((double)clock / rate) / 64 : 0;
	F2151->ST.TimerBase = 1.0/((double)clock / 64.0);
	OPMInitTable();
}

/* ---- state save/load (REGS mirror + replay) ---- */
...
int opm_save_state(BYTE *buf, int maxlen)
{
	...
	/* Register mirror */
	SS_WRITE(buf, pos, OPM->REGS, 256);
	/* ST fields that OPMWriteReg does not restore */
	SS_WRITE_U8(buf, pos, OPM->ST.address);
	SS_WRITE_U8(buf, pos, OPM->ST.status);
	SS_WRITE_U8(buf, pos, OPM->ST.irq);
	SS_WRITE_U8(buf, pos, OPM->ST.irqmask);
	SS_WRITE_U8(buf, pos, OPM->ST.mode);
	/* timer values */
	SS_WRITE_VAL(buf, pos, OPM->ST.TA);
	SS_WRITE_VAL(buf, pos, OPM->ST.TAC);
	SS_WRITE_VAL(buf, pos, OPM->ST.TB);
	SS_WRITE_VAL(buf, pos, OPM->ST.TBC);
	return pos;
}
...
int opm_load_state(const BYTE *buf, int len)
{
	...
	int r;
	UINT8 regs[256];
	UINT8 address, status, irq, irqmask, mode;
	YM2151 *OPM = &FMOPM[0];

	/* Read saved data */
	SS_READ(buf, pos, regs, 256);
	SS_READ_U8(buf, pos, address);
	SS_READ_U8(buf, pos, status);
	SS_READ_U8(buf, pos, irq);
	SS_READ_U8(buf, pos, irqmask);
	SS_READ_U8(buf, pos, mode);

	int TA, TAC, TBC;
	UINT8 TB;
	SS_READ_VAL(buf, pos, TA);
	SS_READ_VAL(buf, pos, TAC);
	SS_READ_VAL(buf, pos, TB);   /* ST.TB is UINT8, not int */
	SS_READ_VAL(buf, pos, TBC);

	/* Reset chip then replay registers (YM2151_postload pattern) */
	OPMResetChip(0);

	/* Copy REGS mirror first */
	memcpy(OPM->REGS, regs, 256);

	/* Replay in same order as YM2151_postload */
	OPMWriteReg(0, 0x0f, regs[0x0f]);	/* noise sel */
	OPMWriteReg(0, 0x18, regs[0x18]);	/* lfreq */
	OPMWriteReg(0, 0x1b, regs[0x1b]);	/* CT, W */
	for (r = 0xff; r >= 0x20; r--)
		OPMWriteReg(0, r, regs[r]);
	FM_channel_postload(OPM->CH, 8);
	cur_chip = NULL;

	/* Restore fields not touched by OPMWriteReg */
	OPM->ST.address = address;
	OPM->ST.status = status;
	OPM->ST.irq = irq;
	OPM->ST.irqmask = irqmask;
	OPM->ST.mode = mode;
	OPM->ST.TA = TA;
	OPM->ST.TAC = TAC;
	OPM->ST.TB = TB;
