...
- SLANG source embedding: select a valid placeholder value such as `$00`, then use the existing **Import** button to replace it with binary array values without sending the bytes through the AI. For a runtime file instead, add the binary to the project disk with Disk Editor and use `MAGLOAD` or `FOPEN`/`FREAD` as appropriate.
