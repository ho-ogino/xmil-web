...
const SERVER_INSTRUCTIONS = [
  ...
  'Before sending a replacement, treat it as large asset-like only when it embeds substantially all of a known asset of at least 8 KiB, or has at least 8,192 byte literals in table-like runs covering at least 50% of its non-whitespace text; source length alone never triggers this rule. Before explicit user approval, do not read, re-emit, or split that data through the model. Offer ASM: the existing Import button for DB lines; SLANG: the existing Import button replaces a selected placeholder with array values, or use Disk Editor plus MAGLOAD/FOPEN/FREAD; BASIC: Disk Editor plus BLOAD or the applicable file workflow. Do not suggest local UTF-8 source-file sync unless available and user-configured, and then only for a complete prepared source section, not raw binary or a fragment. After approval, prefer one write and split only if the client output limit requires it.',
